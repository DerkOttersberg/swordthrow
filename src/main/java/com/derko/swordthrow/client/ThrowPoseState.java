...
public final class ThrowPoseState {
    ...
    public static void applyThirdPersonOffHandPose(float age, HumanoidArm offArmSide, ModelPart offArm) {
        ...
        PoseSample pose = sample(age, offArmSide.getOpposite(), age - Mth.floor(age));
    ...
    public static void applyThirdPersonMainHandPose(float age, HumanoidArm mainArmSide, ModelPart mainArm) {
        ...
        PoseSample pose = sample(age, mainArmSide, age - Mth.floor(age));
    ...
    private static float getReleaseProgress(float tickDelta) {
        ...
        return ease(Mth.clamp(raw, 0.0F, 1.0F));
    }
    ...
    private static PoseSample sample(float age, HumanoidArm mainArm, float tickDelta) {
        ...
        float mainSwaySide = Mth.sin(time * 0.18F) * swayWeight;
        float mainSwayLift = Mth.cos(time * 0.13F) * swayWeight;
        float mainSwayDepth = Mth.sin(time * 0.11F + 0.8F) * swayWeight;
        float mainSwayRoll = Mth.cos(time * 0.16F + 0.45F) * swayWeight;
        float offSwaySide = Mth.sin(time * 0.13F + 1.1F) * offSwayWeight * 0.45F;
        float offSwayLift = Mth.cos(time * 0.11F + 0.4F) * offSwayWeight * 0.20F;
        float offSwayDepth = Mth.sin(time * 0.09F + 2.0F) * offSwayWeight * 0.22F;
        float offSwayRoll = Mth.cos(time * 0.12F + 1.7F) * offSwayWeight * 0.55F;
        ...
        return new PoseSample(
            mainArm == HumanoidArm.RIGHT ? 1.0F : -1.0F,
            mainArm == HumanoidArm.RIGHT ? -1.0F : 1.0F,
            windUp,
            snap,
            supportPresence,
    ...
    private static float smoothStep(float edge0, float edge1, float value) {
        float normalized = Mth.clamp((value - edge0) / (edge1 - edge0), 0.0F, 1.0F);
        return normalized * normalized * (3.0F - 2.0F * normalized);
    }

    private static float easeIntoHalfCharge(float charge) {
        float cappedCharge = Mth.clamp(charge, 0.0F, 1.0F);
    ...
    private static final class FloatDriver {
        ...
        private void tick(float targetValue, float response) {
            ...
            currentValue = Mth.lerp(response, currentValue, targetValue);
        ...
        private float get(float tickDelta) {
            return Mth.lerp(tickDelta, previousValue, currentValue);
        }
    }
}
